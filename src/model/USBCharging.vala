...
public class IdeaPadConfigTool.USBCharging : IdeaPadMode {
    ...
    public  int check_status() {
        ...
        if(file_exists() == true) {
            try{
                ...
                int status;
                Process.spawn_command_line_sync (IdeaPadUtils.CMD_USBCHARGING_STATUS, out stdout, out stderr, out status);
                if(status == 0) {
                    stdout = stdout.strip();
    ...
    */
    public bool change_mode(int actual_status) {
		...
		if(actual_status == IdeaPadUtils.STATUS.ENABLE) {
			cmd = IdeaPadUtils.CMD_ENABLE_USBCHARGING;
		} else if(actual_status == IdeaPadUtils.STATUS.DISABLE) {
			cmd = IdeaPadUtils.CMD_DISABLE_USBCHARGING;
		}
    ...
    */
    ...
    public void send_on_notification() {
        Notification.prepared("usb-charging", "on");
    }
    public void send_off_notification() {
        Notification.prepared("usb-charging", "off");
    }

    public void send_fail_notification() {
        Notification.prepared("usb-charging", "fail");
    }
