...
public class IdeaPadConfigTool.BatteryConservative : IdeaPadMode {
    ...
    */
    ...
    /*
        Get the content of file conservation_mode over cat command and bring back their value like an integer.
    ...
    public  int check_status() {
        ...
        if(file_exists() == true) {
            try{
                ...
                int status;
                Process.spawn_command_line_sync (IdeaPadUtils.CMD_BATT_STATUS, out stdout, out stderr, out status);
                if(status == 0) {
                    stdout = stdout.strip();
    ...
    */
    public bool change_mode(int actual_status) {
		...
		if(actual_status == IdeaPadUtils.STATUS.ENABLE) {
			cmd = IdeaPadUtils.CMD_ENABLE_BATT;
		} else if(actual_status == IdeaPadUtils.STATUS.DISABLE) {
			cmd = IdeaPadUtils.CMD_DISABLE_BATT;
		}
    ...
    */
    ...
    public void send_on_notification() {
        Notification.prepared("battery-mode", "on");
    }
    public void send_off_notification() {
        Notification.prepared("battery-mode", "off");
    }

    public void send_fail_notification() {
        Notification.prepared("battery-mode", "fail");
    }
